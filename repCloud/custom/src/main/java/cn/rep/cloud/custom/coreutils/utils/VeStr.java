...
//	}
	...
	/**
	 * 获取姓，名
	 * @param fullname 全名字符串
	 * @return {firstname,lastname}
	 */
	public static String[] getFullName(String fullname){
		String firstname = "";
		String lastname = "";
		String[] bai={"欧阳","太史","上官","端木","司马","东方","独孤","南宫","万俟","闻人",
					  ...
					  "第五","公乘","贯丘","公皙","南荣","东里","东宫","仲长","子书","子桑","即墨","达奚","褚师","吴铭"};
		int vLength = fullname.length();
		if (vLength > 2){
			String preTwoWords = fullname.substring(0, 2);
			// 取命名的前两个字,看是否在复姓库中
			boolean flag = false;
			for (int i = 0; i < bai.length; i++) {
				if (preTwoWords.equals(bai[i])) {
					firstname = preTwoWords;
					lastname = fullname.substring(2);
					flag = true;
					break;
				}
			}
		...
		String[] resname = {firstname,lastname};
		return resname;
	}
